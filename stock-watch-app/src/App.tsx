...
const NavbarMoodGauge: React.FC<{ upCount: number; isDark: boolean }> = memo(({ upCount, isDark }) => {
  ...
  return (
    <div
      className={`relative rounded-lg px-3 py-1.5 cursor-help ${isDark ? 'bg-slate-800/80' : 'bg-white border border-gray-200 shadow-sm'}`}
      onMouseEnter={() => setShowTooltip(true)}
      onMouseLeave={() => setShowTooltip(false)}
    >
      {/* Tooltip - Below */}
      {showTooltip && (
        <div className={`absolute top-full left-1/2 -translate-x-1/2 mt-2 w-64 p-3 rounded-lg shadow-xl z-50 text-xs ${
          isDark ? 'bg-slate-700 text-white border border-slate-600' : 'bg-white text-gray-800 border border-gray-200'
        }`}>
          {/* Arrow pointing up */}
          <div className={`absolute bottom-full left-1/2 -translate-x-1/2 border-8 border-transparent ${
            isDark ? 'border-b-slate-700' : 'border-b-white'
          }`} />
          <div className="font-semibold mb-1.5">Market Mood Indicator</div>
          <p className={`mb-2 ${isDark ? 'text-slate-300' : 'text-gray-600'}`}>
            Tracks the 3 major US indices: S&P 500, Nasdaq, and Dow Jones.
          </p>
          <div className="space-y-1">
            <div className="flex items-center gap-2">
              <span className="text-green-500">🚀</span>
              <span><strong>3/3 up:</strong> Strong bullish momentum</span>
            </div>
            <div className="flex items-center gap-2">
              <span className="text-green-400">📈</span>
              <span><strong>2/3 up:</strong> Bullish bias</span>
            </div>
            <div className="flex items-center gap-2">
              <span className="text-yellow-400">⚖️</span>
              <span><strong>1/3 up:</strong> Mixed signals</span>
            </div>
            <div className="flex items-center gap-2">
              <span className="text-red-400">🐻</span>
              <span><strong>0/3 up:</strong> Bearish momentum</span>
            </div>
          </div>
          <div className={`mt-2 pt-2 border-t ${isDark ? 'border-slate-600 text-slate-400' : 'border-gray-200 text-gray-500'}`}>
            💡 Use as a quick sentiment check, not a trading signal.
          </div>
        </div>
      )}
...
const NavbarVixGauge: React.FC<{ vixValue: number; isDark: boolean }> = memo(({ vixValue, isDark }) => {
  ...
  return (
    ...
    >
      {/* Tooltip - Below */}
      {showTooltip && (
        <div className={`absolute top-full left-1/2 -translate-x-1/2 mt-2 w-72 p-3 rounded-lg shadow-xl z-50 text-xs ${
          isDark ? 'bg-slate-700 text-white border border-slate-600' : 'bg-white text-gray-800 border border-gray-200'
        }`}>
          {/* Arrow pointing up */}
          <div className={`absolute bottom-full left-1/2 -translate-x-1/2 border-8 border-transparent ${
          ...
          <div className="font-semibold mb-1.5">CBOE Volatility Index (VIX)</div>
          <p className={`mb-2 ${isDark ? 'text-slate-300' : 'text-gray-600'}`}>
            The "Fear Index" — measures expected market volatility over the next 30 days based on S&P 500 options.
          </p>
          <div className="space-y-1">
            <div className="flex items-center justify-between">
              <span className="text-green-500 font-medium">0-15: Calm</span>
              <span className={isDark ? 'text-slate-400' : 'text-gray-500'}>Complacency, low fear</span>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-blue-500 font-medium">15-20: Normal</span>
              <span className={isDark ? 'text-slate-400' : 'text-gray-500'}>Typical conditions</span>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-yellow-500 font-medium">20-25: Elevated</span>
              <span className={isDark ? 'text-slate-400' : 'text-gray-500'}>Uncertainty rising</span>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-orange-500 font-medium">25-30: High</span>
              <span className={isDark ? 'text-slate-400' : 'text-gray-500'}>Significant fear</span>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-red-500 font-medium">30+: Extreme</span>
              <span className={isDark ? 'text-slate-400' : 'text-gray-500'}>Panic selling</span>
            </div>
          </div>
          <div className={`mt-2 pt-2 border-t ${isDark ? 'border-slate-600 text-slate-400' : 'border-gray-200 text-gray-500'}`}>
            💡 High VIX can signal buying opportunities for long-term investors.
          </div>
        </div>
      )}
