...
export function Search() {
  ...
  // Subscription status - AI features require Pro
  const { canAccessAI, isLoading: subscriptionLoading } = useSubscription();
  ...
  const handleSearch = async (searchQuery: string) => {
    ...
    try {
      ...
      // Auto-generate narrative if in AI mode, have results, AND user has Pro subscription
      if (viewMode === 'ai' && searchResult.results.length > 0 && canAccessAI) {
  ...
  const handleViewModeChange = async (mode: ViewMode) => {
    setViewMode(mode);
    // Only generate narrative if user has Pro subscription
    if (mode === 'ai' && results.length > 0 && !narrative && !generatingNarrative && canAccessAI) {
      setGeneratingNarrative(true);
      try {
          ...
          <div>
            ...
            {viewMode === 'ai' && (
              <div className="mb-6">
                ...
                {!canAccessAI && !subscriptionLoading ? (
                  <div className="mb-4">
                    <UpgradePrompt feature="aiNarrative" />
                    <p className="text-sm text-gray-500 dark:text-gray-400 text-center mt-4">
                      You can still view search results in List view below.
                    </p>
                  </div>
                ) : generatingNarrative ? (
