...
/**
 * Calculate text match score (simple keyword matching)
 */
function calculateTextScore(searchText: string, queryWords: string[]): number {
  ...
  const weightedScore = totalWeight / (queryWords.length * 1.5);
...
export async function searchSegments(
  ...
  const { groupId, limit = 10, minScore = 0.2, types } = options;
  ...
  try {
    ...
    const scoredSegments = segments.map((segment) => {
      ...
      const score = calculateTextScore(searchText, queryWords);
